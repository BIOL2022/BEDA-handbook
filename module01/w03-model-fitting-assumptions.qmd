---
title: "Model fitting and assumptions"
...
Use this resource while working through the Week 3 practical. It provides worked examples of fitting models and checking their assumptions.

When we fit a model under the general linear model framework, we make assumptions about the data. If those assumptions are violated, the results of our tests might not be reliable.

The examples use R code and Jamovi output, so you can use the approach that best fits your group’s workflow. We use the `palmerpenguins` dataset, which contains data on three penguin species from the Palmer Archipelago in Antarctica.
...
## 1. A general linear model with a *continuous* predictor

Let's examine the relationship between a penguin's body mass and its flipper length. We begin by thinking about how these two variables might be related and construct the model. Words are fine; the general linear model will work out the details once it is fitted!
...
We can generate a set of standard diagnostic plots in R, Jamovi and most other statistical software packages when we fit a general linear model. For example, below is an output in R.
...
#| cache: true
#| echo: false
#| fig-cap: "Diagnostic plots for the linear model of penguin body mass predicted by flipper length. The plots are produced in R."
#| fig-alt: "Four diagnostic plots for a linear model of penguin body mass by flipper length."
...
::: {.column-margin}
### <i class="bi bi-info-circle me-1" aria-hidden="true"></i> Interpreting the plots
...
![QQ-plot](assets/wt_penguin_qqplot.svg){width=230 fig-alt="Q-Q plot for Jamovi residual diagnostics."}

![Residuals vs Fitted](assets/wt_penguin_residuals_fitted.svg){width=300 fig-alt="Residuals versus fitted values plot for Jamovi diagnostics."}

Diagnostic plots for the linear model of penguin body mass predicted by flipper length. The plots are produced in Jamovi.
:::

## 2. A general linear model with a *categorical* predictor
...
#| fig-alt: "Four diagnostic plots for a linear model of penguin bill length by sex."
par(mfrow = c(2, 2))
plot(ttest_lm_fit)
par(mfrow = c(1, 1))
```
